...
const O200K_BASE_SHA256 = "446a9538cb6c348e3516120d7c08b09f57c36495e2acfffe59a5bf8b0cfb1a2d";
...
export class TokenCodec {
    ...
    encode(text) {
        ...
        for (const match of text.matchAll(pattern)) {
            ...
            if (matchIndex !== consumedUtf16Units) {
                throw new Error(`Tokenizer regex left unmatched text at UTF-16 offset ${consumedUtf16Units}.`);
            }
        ...
        if (consumedUtf16Units !== text.length) {
            throw new Error(`Tokenizer regex consumed ${consumedUtf16Units} of ${text.length} UTF-16 units.`);
        }
        return result;
    }
    decode(tokens) {
    ...
    tokenBytes(token) {
        ...
        if (!bytes)
            throw new Error(`Unknown ${this.name} token id: ${token}`);
        return bytes;
    }
}
function loadEncoding(name) {
    ...
    if (sourceHash !== O200K_BASE_SHA256) {
        throw new Error(`Unexpected ${name} SHA-256: ${sourceHash}; expected ${O200K_BASE_SHA256}.`);
    }
    const ranks = new Map();
    const inverseRanks = new Map();
    for (const line of source.split("\n")) {
        if (line.length === 0)
            continue;
        const separator = line.lastIndexOf(" ");
        if (separator <= 0)
            throw new Error(`Invalid tokenizer rank line: ${line}`);
    ...
    if (ranks.size !== 199_998) {
        throw new Error(`Unexpected o200k_base vocabulary size: ${ranks.size}; expected 199998.`);
    }
    const data = { ranks, inverseRanks };
    encodingCache.set(name, data);
    return data;
}
function readEncodingFile(name) {
    ...
    if (!path) {
        throw new Error(`Cannot find ${filename}. Tried: ${candidates.join(", ")}. ` +
...
function bytePairEncode(piece, ranks) {
    ...
    if (piece.length === 1) {
        ...
        if (rank === undefined)
            throw new Error("Missing single-byte tokenizer rank.");
        return [rank];
    }
    const parts = Array.from({ length: piece.length }, (_, index) => ({ start: index, end: index + 1 }));
    while (parts.length > 1) {
        ...
        if (!left || !right)
            throw new Error("Invalid BPE merge state.");
    ...
    return parts.map(({ start, end }) => {
        ...
        if (rank === undefined)
            throw new Error("Tokenizer produced an unknown byte piece.");
        return rank;
    });
}
