...
export function trainSharedDictionary(samples, options = {}) {
    ...
    const entries = optimized.document.dictionary
...
export function createSharedDictionary(id, version, entries, encoding = "o200k_base") {
    const codec = new TokenCodec(encoding);
    const seenAliases = new Set();
    const seenValues = new Set();
    const normalized = [];
    for (const entry of entries) {
        if (!entry.alias || !entry.value)
            continue;
        if (seenAliases.has(entry.alias) || seenValues.has(entry.value))
            continue;
        if (codec.count(entry.alias) > codec.count(entry.value))
            continue;
...
export function applySharedDictionary(input, dictionary) {
    ...
    for (const entry of ranked) {
        if (body.includes(entry.alias))
            continue;
        const occurrences = countNonOverlapping(body, entry.value);
        if (occurrences === 0)
            continue;
        if (occurrences * (codec.count(entry.value) - codec.count(entry.alias)) <= 0)
            continue;
...
export function expandSharedDictionary(input, dictionary) {
    let value = input;
...
export function renderSharedBootstrap(dictionary) {
...
export function parseSharedDictionary(serialized) {
...
export function serializeSharedDictionary(dictionary) {
    return `${JSON.stringify(dictionary, null, 2)}\n`;
}
export function chooseStableAliases(count) {
    return BALANCED_ALIAS_SYMBOLS.slice(0, Math.max(0, count));
}
function finalizeDictionary(dictionary) {
...
export function toDictionaryEntries(entries) {
