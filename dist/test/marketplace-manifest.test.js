...
test("Codex marketplace uses accepted policy values and a contained source path", () => {
    ...
    const plugin = marketplace.plugins.find((entry) => entry.name === "ltcf-prompt-compression");
    assert.ok(plugin, "LTCF marketplace entry is missing");
    assert.ok(["ON_INSTALL", "ON_USE"].includes(plugin.policy.authentication), `unsupported authentication policy: ${plugin.policy.authentication}`);
...
//# sourceMappingURL=marketplace-manifest.test.js.map