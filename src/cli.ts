...
function transformCommand(args: string[]): void {
...
function trainCommand(args: string[]): void {
  ...
  if (files.length === 0) throw new Error("train requires corpus files after --");
...
function bootstrapCommand(args: string[]): void {
  const dictionaryPath = valueAfter(args, "--dictionary");
  if (!dictionaryPath) throw new Error("bootstrap requires --dictionary <file>");
...
function observeCommand(args: string[]): void {
...
function exportCommand(args: string[]): void {
  ...
  if (!dictionary) throw new Error("No trained dictionary exists in the learning state yet.");
...
async function runCommand(args: string[]): Promise<void> {
  ...
  if (divider === -1 || divider === args.length - 1) throw new Error("run requires -- <command> [args...]");
  ...
  if (!command) throw new Error("Missing child command.");
...
function benchmarkCommand(args: string[]): void {
  const inputPath = valueAfter(args, "--input");
  if (!inputPath) throw new Error("benchmark requires --input <file>");
  const dictionaryPath = valueAfter(args, "--dictionary");
...
function summary(result: ReturnType<typeof transformPrompt>): Record<string, unknown> {
  return {
...
function positionalAfterDoubleDash(args: string[]): string[] {
