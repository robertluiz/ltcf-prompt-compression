...
interface ProfileConfiguration {
  maxNgramCandidates: number[];
  maxEntries: number;
  candidateLimit: number;
  exactCandidatesPerRound: number;
}

const PROFILES: Record<OptimizationProfile, ProfileConfiguration> = {
  balanced: {
 ...
 */
export function optimizeCompression(
  original: string,
  ...
  const candidateLengths = [...new Set(
    selectedProfile.maxNgramCandidates
      .map((value) => Math.min(value, Math.max(2, sourceTokens - 1)))
      .filter((value) => value >= 2),
  )];

  let bestDocument: ReturnType<typeof compress> | undefined;
  let bestMetrics: ReturnType<typeof metrics> | undefined;
